...
const char* folderName = "/music/tiletest/";
...
uint32_t completeBuffer = 0;         // Which section is full
...
inline uint8_t myMixSound()
{
    ...
    if(playBGM & playingMusic1) {number = (audioBuffer[audioOffset]-128); divide++;}
    int currentPos;
    // check the snd channels for playing audio
    ...
    if(!divide){ // divide is how many samples have been added together
        //number=0;
        return 128;
    }else{
